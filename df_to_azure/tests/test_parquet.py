...
def test_upsert_parquet_same_shape():
    ...
    df_to_azure(df=df, tablename="upsert", schema="test_parquet", parquet=True)
    ...
    # perform upsert
    df_to_azure(df=df, tablename="upsert", schema="test_parquet", method="upsert", parquet=True, id_field=["id"])
    ...
    downloaded_blob = container_client.download_blob("test_parquet/upsert.parquet")
