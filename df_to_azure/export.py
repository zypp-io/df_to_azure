...
from df_to_azure.utils import test_unique_column_names, test_uniqueness_column, wait_until_pipeline_is_done
...
class DfToAzure(ADF):
    ...
    def upload_dataset(self):
        ...
        if self.method == "upsert":
            # Key columns need only unqiue values for upsert
            test_uniqueness_column(self.df, self.id_field)
            upsert = SqlUpsert(
